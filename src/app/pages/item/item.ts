...
export class ItemPage extends BasePage {
  ...
  setupObservables() {
    ...
    this.loadAndScroll = merge(this.content.ionScroll, this.contentLoaded);
  }
  ...
  async ionViewDidEnter() {
    try {
      ...
      if (this.item.hasVariations()) {
        this.variations = this.item.variations.filter(
      ...
      if (this.item.description) {
        this.itemDescription = this.sanitizer.bypassSecurityTrustHtml(
  ...
  async onAddToCart() {
    ...
    if (start <= now && now <= end) {
      const alert = await this.alertController.create({
        ...
        header: "Alerte",
        message: "Veuillez commander entre 18h et 3h30 !",
        buttons: [
          {
            text: "OK",
            role: "cancel",
    ...
    try {
      ...
      if (!User.getCurrent()) {
        ...
        const user = await this.userService.becomeWithSessionToken(
          res.getSessionToken()
        );
      ...
      const existInCart = cart.items.find((item: any) => {
        if (item.variation) {
          return (
            item.objectId === filteredItem.objectId &&
            item.variation.objectId === filteredItem.variation.objectId
          );
        }
