...
export class CheckoutPage extends BasePage {
  ...
  showCouponErrorsIfNeeded(error: any) {
    if (error.code === 5000) {
      ...
      this.translate
        .get("COUPON_USAGE_LIMIT_REACHED")
        .subscribe((str) => this.showToast(str));
    } else if (error.code === 5003) {
      this.translate
        .get("COUPON_NOT_STARTED_YET")
        .subscribe((str) => this.showToast(str));
      ...
      this.translate
        .get("COUPON_USAGE_LIMIT_PER_USER_REACHED")
        .subscribe((str) => this.showToast(str));
      ...
      if (matches) {
        ...
        this.translate
          .get("ERROR_MINIMUM_AMOUNT_COUPON", { value: formattedValue })
          .subscribe((str) => this.showToast(str));
      }
      ...
      if (matches) {
        ...
        const formattedValue = this.currencyGlobalPipe.transform(value);
        this.translate
          .get("ERROR_MAXIMUM_AMOUNT_COUPON", { value: formattedValue })
          .subscribe((str) => this.showToast(str));
      }
    } else if (
      error.code === 5008 ||
  ...
  async onPlaceOrder() {
    try {
      ...
      if (err.lastIndexOf("insufficient") > 0) {
        this.showToast("Votre carte n'est pas suffisamment provisionnée");
        ...
        if (matches) {
          ...
          const formattedValue = this.currencyGlobalPipe.transform(value);
          this.translate
            .get("ERROR_MINIMUM_ORDER_AMOUNT", { value: formattedValue })
            .subscribe((str) => this.showToast(str));
        }
        ...
        if (matches) {
          ...
          const formattedValue = this.currencyGlobalPipe.transform(value);
          this.translate
            .get("ERROR_MAXIMUM_ORDER_AMOUNT", { value: formattedValue })
            .subscribe((str) => this.showToast(str));
        }
      } else if (error.code === 1006) {
        this.onPresentSignUpModal();
        this.translate
          .get("ERROR_CHECKOUT_GUEST_DISABLED")
